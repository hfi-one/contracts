...
contract HfiDao is IERC20, Context {
    ...
    constructor(
        ...
        _name = __name;
        _symbol = __name;
        _decimals = IERC20(_HFI).decimals();
    ...
    function deposit(uint256 _amount) public onlyHuman {
        require(_amount > 0);
        uint256 _before = IERC20(HFI).balanceOf(address(this));
        IERC20(HFI).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 _after = IERC20(HFI).balanceOf(address(this));
    ...
    function withdraw(uint256 _shares) public onlyHuman {
        ...
        IERC20(HFI).safeTransfer(msg.sender, r);
    }
