...
interface ICurve {
    function add_liquidity(uint[2] memory amts, uint minAmt) external;
}
...
contract StrategyHBOLp {
    ...
    address public want = 0x8b65b86dc3CaDcA97b50D4757BB8A686E6ea0cE1;

    address public DEP = 0x48C859531254F25e57D1C1A8E030Ef0B1c895c27;

    ICurve public curve = ICurve(0x11bFEE9D8625ac4cDa6Ce52EeBF5caC7DC033d15);
    ...
    function harvest() public onlyBenevolent {
        ...
        uint256 _reward = IERC20(husd).balanceOf(address(this)).mul(strategistReward).div(FEE_DENOMINATOR);
        IERC20(husd).safeTransfer(
            IController(controller).rewards(),
            _reward
        );

        uint remain = IERC20(husd).balanceOf(address(this));
        if (remain == 0) {
            return;
        }

        uint[2] memory amts = [remain, 0];
        IERC20(husd).safeApprove(address(curve), 0);
        IERC20(husd).safeApprove(address(curve), uint(-1));
        curve.add_liquidity(amts, 0);
