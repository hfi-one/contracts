...
contract StrategyHBOLp {
    ...
    address public constant HBO = 0x8764Bd4fcc027faF72bA83c0b2028a3BAE0D2D57;
    ...
    function withdraw(IERC20 _asset) external returns (uint256 balance) {
        ...
        require(HBO != address(_asset), "want");
    ...
    function harvest() public onlyBenevolent {
        ...
        uint256 _hbo = IERC20(HBO).balanceOf(address(this));

        address[] memory path = new address[](2);
        if (_hbo > 0) {
            uint256 _reward = _hbo.mul(strategistReward).div(FEE_DENOMINATOR);

            IERC20(HBO).safeApprove(uniRouter, 0);
            IERC20(HBO).safeApprove(uniRouter, uint256(-1));
        ...
        _hbo = IERC20(HBO).balanceOf(address(this));
        if (_hbo == 0) {
            return;
        }
        ...
        Uni(uniRouter).addLiquidity(
            HBO,
            pairToken,
            IERC20(HBO).balanceOf(address(this)),
    ...
    function balanceOfPool() public view returns (uint256) {
        return IPool(pool).balanceOf(address(this));
    }
