...
interface IPowerProvider {
    function getPower(address user) external view returns (uint);
}

contract mfVault is ERC20, ERC20Detailed {
    ...
    constructor (address _token, address _controller, bool _earnImmediately, address _powerProvider) ERC20Detailed(
        string(abi.encodePacked("MHECOYFI ", ERC20Detailed(_token).name())),
        string(abi.encodePacked("mf", ERC20Detailed(_token).symbol())),
    ...
    function updatePowerProvider(address p) public {
        require(msg.sender == governance, "!governance");
        powerProvider = p;
    }

    function updateRequiredPower(uint p) public {
        require(msg.sender == governance, "!governance");
        requiredPower = p;
    }
    ...
    function deposit(uint _amount) public payable onlyHuman {
        ...
        require(IPowerProvider(powerProvider).getPower(msg.sender) >= requiredPower, "!power");
